...
# it also includes a method "plot_learning_curves" that launches a separate CPU process
# that plots the Training and Validation learning curves live
class Kregistrar(Callback):
    ...
    def on_train_end(self, logs={}):
        if self.reporting_freq:
            self.report()
        printflush(
            '\nModel Training Finished! (%s Batches in total)\n'
        ...
        if self.latest_val_acc is None:
            ...
            printflush(
                'Best trained Model (with lowest Validation Loss) is from epoch #%s'
